...
--
-- On windows, this expects valid UCS-2LE, on unix it is passed unchanged/unchecked.
--
-- Throws 'EncodingException' on invalid UCS-2LE on windows (although unlikely).
--
-- Note: This is rarely what you want, because you have to ensure the input will
--       work on both platforms (unix and windows) equally (Word16 vs Word8 array).
--       This is unlikely to be the case, unless you get the raw bytes via FFI
--       from 'unix' and 'Win32' package or manually construct correct platform
--       specific ByteStrings.
unsafeFromBytes :: MonadThrow m
                => ByteString
                -> m OsString
unsafeFromBytes = fmap OsString . PF.unsafeFromBytes
