...
import qualified System.OsString.Types.Internal as OST
...
main = defaultMain
    ...
    , bgroup "encoding/decoding" $ map (uncurry bench)
      ...
      , ("unsafeFromBytes (posix)"             , nf (OSP.unsafeFromBytes @Maybe) (SBS.fromShort . OST.getPosixString $ posixPathAFPP))
      , ("unsafeFromBytes (windows)"           , nf (WSP.unsafeFromBytes @Maybe) (SBS.fromShort . OST.getWindowsString $ windowsPathAFPP))
      ]
    ]
