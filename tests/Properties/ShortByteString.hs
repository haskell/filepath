...
import qualified System.FilePath.Data.ByteString.Short.Word16 as B
import System.FilePath.Data.ByteString.Short.Internal (isSpace, _nul)
...
tests =
  ...
  , testProperty "splitWith length" $
    \f x -> let splits = B.splitWith f x; l1 = fromIntegral (length splits); l2 = B.numWord16 (B.filter f x) in
      (l1 == l2 || l1 == l2 + 1) && sum (map B.numWord16 splits) + l2 == B.numWord16 x
  ...
  , testProperty "tail length" $
    \x -> not (B.null x) ==> B.numWord16 x === 1 + B.numWord16 (B.tail x)
  , testProperty "init" $
    \x -> not (B.null x) ==> B.unpack (B.init x) === init (B.unpack x)
  , testProperty "init length" $
    \x -> not (B.null x) ==> B.numWord16 x === 1 + B.numWord16 (B.init x)
  ...
  , testProperty "index" $
    \(NonNegative n) x -> fromIntegral n < B.numWord16 x ==> B.index x (fromIntegral n) === B.unpack x !! n
  , testProperty "indexMaybe" $
    \(NonNegative n) x -> fromIntegral n < B.numWord16 x ==> B.indexMaybe x (fromIntegral n) === Just (B.unpack x !! n)
  , testProperty "indexMaybe Nothing" $
    \n x -> (n :: Int) < 0 || fromIntegral n >= B.numWord16 x ==> B.indexMaybe x (fromIntegral n) === Nothing
  , testProperty "!?" $
