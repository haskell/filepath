...
import qualified Data.ByteString.Short.Internal as BS8
import qualified System.OsPath.Data.ByteString.Short.Internal as BS8
...
-- with the small distinction that we're not truncating at NUL bytes (because we're not at
-- the outer FFI layer).
decodeWithBaseWindows :: BS8.ShortByteString -> IO String
decodeWithBaseWindows ba = BS8.useAsCWStringLen ba $ \fp -> peekFilePathWin fp

-- | This mimics the filepath dencoder base uses on windows,
-- with the small distinction that we're not truncating at NUL bytes (because we're not at
-- the outer FFI layer).
encodeWithBaseWindows :: String -> IO BS8.ShortByteString
encodeWithBaseWindows str = withFilePathWin str $ \l cstr -> BS8.packCWStringLen (cstr, l)
